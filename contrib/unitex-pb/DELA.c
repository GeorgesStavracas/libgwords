...
while (res->n_semantic_codes<MAX_SEMANTIC_CODES && s[i]=='+') {
  ...
  temp[j]='\0';
  res->semantic_codes[(uint)res->n_semantic_codes]=(unichar*)malloc(sizeof(unichar)*(u_strlen(temp)+1));
  if (res->semantic_codes[(uint)res->n_semantic_codes]==NULL) {
    fprintf(stderr,"Memory allocation error!\n");
    return NULL;
  }
  u_strcpy(res->semantic_codes[(uint)res->n_semantic_codes],temp);
...
while (res->n_flexional_codes<MAX_FLEXIONAL_CODES && s[i]==':') {
  ...
  temp[j]='\0';
  res->flexional_codes[(uint)res->n_flexional_codes]=(unichar*)malloc(sizeof(unichar)*(u_strlen(temp)+1));
  if (res->flexional_codes[(uint)res->n_flexional_codes]==NULL) {
    fprintf(stderr,"Memory allocation error!\n");
    return NULL;
  }
  u_strcpy(res->flexional_codes[(uint)res->n_flexional_codes],temp);
...
while (s[i]!='\0' && s[i]!='+' && s[i]!=':' && s[i]!='/') {
  if (s[i]=='\t') {
     sprintf(err,"Line %d: tabulation in gramatical or sematic code\n",line);
     u_fprints_char(err,out);
     u_fprints(s,out);
     u_fprints_char("\n",out);
     return;
